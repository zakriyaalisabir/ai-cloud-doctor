...
export async function analyzeTf(cfg: AppConfig, opts: Record<string, any>): Promise<string> {
  ...
  const response = await openai.ask(
    ...
    opts.question || "Analyze this Terraform plan for risks and recommendations"
  );
  ...
  console.log(`\n🛠️ Tokens: ${response.inputTokens} in, ${response.outputTokens} out, ${response.cachedTokens} cached | Job: ${jobId}`);
